...
def tree_apply(fn, tree_node):
  if isinstance(tree_node, dict):
    return {k: tree_apply(tree_node[k], fn) for k in tree_node}
  if isinstance(tree_node, list):
    return [tree_apply(elem, fn) for elem in tree_node]
  if isinstance(tree_node, tuple):
    return tuple([tree_apply(elem, fn) for elem in tree_node])
  return fn(tree_node)
